import { $ as viewport_default, Ct as log_default, G as Geometry, J as uid, K as BufferTransform, S as AttributeManager, g as layer_default, h as composite_layer_default, ot as picking_default, pt as COORDINATE_SYSTEM, q as Model, ut as project32_default, w as createIterable, x as compareProps, xt as getPassthroughFS, yt as Matrix4, z as deepEqual } from "./dist-dvxpIib5.js";
import "./webgl-device-BEeceotL.js";
import { g as CubeGeometry, l as column_layer_default, o as solid_polygon_layer_default, s as path_layer_default } from "./dist-CNeEc3_D.js";

//#region node_modules/@luma.gl/engine/dist/compute/texture-transform.js
...
var TextureTransform = class {
	...
	destroy() {
		this.model.destroy();
		for (const binding of this.bindings) binding.framebuffer?.destroy();
	}
	...
	_updateBinding(binding, { sourceBuffers, sourceTextures, targetTexture }) {
		...
		if (targetTexture) {
			...
			if (binding.framebuffer) binding.framebuffer.destroy();
	...
	_setSourceTextureParameters() {
		...
		for (const name in sourceTextures) sourceTextures[name].sampler = this.sampler;
	}
};

//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/common/aggregator/cpu-aggregator/sort-bins.js
...
function packBinIds({ bins, dimensions, target }) {
	...
	if (!target || target.length < targetLength) target = new Float32Array(targetLength);
	for (let i = 0; i < bins.length; i++) {
		const { id } = bins[i];
		if (Array.isArray(id)) target.set(id, i * dimensions);
		else target[i] = id;
	}
	return target;
}
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/common/aggregator/cpu-aggregator/aggregate.js
var count = (pointIndices) => {
	return pointIndices.length;
};
var sum = (pointIndices, getValue) => {
	let result = 0;
	for (const i of pointIndices) result += getValue(i);
	return result;
};
var mean = (pointIndices, getValue) => {
	if (pointIndices.length === 0) return NaN;
...
function aggregate({ bins, getValue, operation, target }) {
	if (!target || target.length < bins.length) target = new Float32Array(bins.length);
	let min$1 = Infinity;
	let max$1 = -Infinity;
	for (let j = 0; j < bins.length; j++) {
		const { points } = bins[j];
		target[j] = operation(points, getValue);
		if (target[j] < min$1) min$1 = target[j];
		if (target[j] > max$1) max$1 = target[j];
	}
	return {
		value: target,
		domain: [min$1, max$1]
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/common/aggregator/cpu-aggregator/vertex-accessor.js
...
function evaluateVertexAccessor(accessor, attributes, options) {
	...
	for (const id of accessor.sources || []) {
		...
		if (attribute) vertexReaders[id] = getVertexReader(attribute);
		else throw new Error(`Cannot find attribute ${id}`);
	}
	const data = {};
	return (vertexIndex) => {
		for (const id in vertexReaders) data[id] = vertexReaders[id](vertexIndex);
...
function getVertexReader(attribute) {
	...
	if (size === 1) {
		if (attribute.isConstant) return () => value[0];
		return (vertexIndex) => {
			return value[elementOffset + elementStride * vertexIndex];
		};
	}
	let result;
	...
	return (vertexIndex) => {
		...
		for (let j = 0; j < size; j++) result[j] = value[i + j];
		return result;
	};
}

//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/common/aggregator/cpu-aggregator/cpu-aggregator.js
...
var CPUAggregator = class {
	...
	setProps(props) {
		...
		if (props.binOptions) {
			if (!deepEqual(props.binOptions, oldProps.binOptions, 2)) this.setNeedsUpdate();
		}
		if (props.operations) {
			for (let channel = 0; channel < this.channelCount; channel++) if (props.operations[channel] !== oldProps.operations[channel]) this.setNeedsUpdate(channel);
		}
		if (props.customOperations) {
			for (let channel = 0; channel < this.channelCount; channel++) if (Boolean(props.customOperations[channel]) !== Boolean(oldProps.customOperations[channel])) this.setNeedsUpdate(channel);
		}
		if (props.pointCount !== void 0 && props.pointCount !== oldProps.pointCount) this.setNeedsUpdate();
	...
	*/
	setNeedsUpdate(channel) {
		if (channel === void 0) this.needsUpdate = true;
	...
	update() {
		...
		for (let channel = 0; channel < this.channelCount; channel++) if (this.needsUpdate === true || this.needsUpdate[channel]) {
	...
	getBin(index) {
		...
		for (let i = 0; i < value.length; i++) value[i] = this.results[i]?.value[index];
		return {
			id: bin.id,
			value,
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/common/aggregator/gpu-aggregator/utils.js
/**
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/common/aggregator/gpu-aggregator/bin-sorter-uniforms.js
var uniformBlock$10 = `\
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/common/aggregator/gpu-aggregator/webgl-bin-sorter.js
var COLOR_CHANNELS = [
	1,
	2,
	4,
...
var WebGLBinSorter = class {
	...
	setDimensions(binCount, binIdRange) {
		...
		if (!this.binsFBO) this.binsFBO = createRenderTarget(this.device, width, height);
		else if (this.binsFBO.height < height) this.binsFBO.resize({
			width,
			height
		});
	...
	setModelProps(props) {
		...
		if (props.attributes) model.setAttributes(props.attributes);
		if (props.constantAttributes) model.setConstantAttributes(props.constantAttributes);
		if (props.vertexCount !== void 0) model.setVertexCount(props.vertexCount);
		if (props.shaderModuleProps) model.shaderInputs.setProps(props.shaderModuleProps);
	}
	/** Update aggregation */
	update(operations) {
	...
	_updateBins(operation, colorMask) {
		...
		const initialValue = operation === "MAX" ? -MAX_FLOAT32$1 : operation === "MIN" ? MAX_FLOAT32$1 : 0;
		const renderPass = this.device.beginRenderPass({
...
function getMaskByOperation(operations) {
	...
	for (let channel = 0; channel < operations.length; channel++) {
		...
		if (op) result[op] += COLOR_CHANNELS[channel];
	}
	return result;
}
function createModel(device, props) {
	let userVs = props.vs;
	if (props.dimensions === 2) userVs += `
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/common/aggregator/gpu-aggregator/aggregation-transform-uniforms.js
var uniformBlock$9 = `\
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/common/aggregator/gpu-aggregator/webgl-aggregation-transform.js
...
var WebGLAggregationTransform = class {
	...
	setDimensions(binCount, binIdRange) {
		...
		const aggregatorTransformProps = { binIdRange: [
			binIdRange[0][0],
			binIdRange[0][1],
			binIdRange[1]?.[0] || 0,
			binIdRange[1]?.[1] || 0
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/common/aggregator/gpu-aggregator/webgl-aggregator.js
...
var WebGLAggregator = class {
	...
	getBins() {
		...
		if (this.binIds?.buffer !== buffer) this.binIds = {
			buffer,
			type: "float32",
			size: this.dimensions
		};
	...
	getResult(channel) {
		...
		if (!buffer || channel >= this.channelCount) return null;
		if (this.results[channel]?.buffer !== buffer) this.results[channel] = {
			buffer,
			type: "float32",
			size: 1,
	...
	/** Returns the information for a given bin. */
	getBin(index) {
		if (index < 0 || index >= this.binCount) return null;
		const { binIdRange } = this.props;
		let id;
		if (this.dimensions === 1) id = [index + binIdRange[0][0]];
		else {
		...
		const count$1 = pixel[3];
		const value = [];
		for (let channel = 0; channel < this.channelCount; channel++) {
			const operation = this.props.operations[channel];
			if (operation === "COUNT") value[channel] = count$1;
			else if (count$1 === 0) value[channel] = NaN;
			else value[channel] = operation === "MEAN" ? pixel[channel] / count$1 : pixel[channel];
		}
		return {
			id,
			value,
	...
	setProps(props) {
		...
		if (props.attributes) {
			...
			for (const attribute of Object.values(props.attributes)) for (const [attributeName, value] of Object.entries(attribute.getValue())) if (ArrayBuffer.isView(value)) constantAttributes[attributeName] = value;
		...
		if (props.shaderModuleProps) this.binSorter.setModelProps({ shaderModuleProps: props.shaderModuleProps });
		Object.assign(this.props, props);
	}
	/** Flags a channel to need update.
	* This is called internally by setProps() if certain props change
	...
	*/
	setNeedsUpdate(channel) {
		if (channel === void 0) this.needsUpdate.fill(true);
		else this.needsUpdate[channel] = true;
	}
	update() {}
	/** Run aggregation */
	preDraw() {
		...
		for (let i = 0; i < this.channelCount; i++) if (this.needsUpdate[i]) {
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/common/aggregation-layer.js
var AggregationLayer$1 = class extends composite_layer_default {
	...
	updateState(params) {
		...
		if (params.changeFlags.extensionsChanged || this.state.aggregatorType !== aggregatorType) {
			...
			aggregator.setProps({ attributes: this.getAttributeManager()?.attributes });
			this.setState({
				aggregator,
				aggregatorType
			});
	...
	updateAttributes(changedAttributes) {
		...
		aggregator.setProps({ attributes: changedAttributes });
		for (const id in changedAttributes) this.onAttributeChange(id);
		aggregator.update();
	}
...
AggregationLayer$1.layerName = "AggregationLayer";
var aggregation_layer_default = AggregationLayer$1;
...
function colorRangeToFlatArray(colorRange, normalize = false, ArrayType = Float32Array) {
	let flatArray;
	if (Number.isFinite(colorRange[0])) flatArray = new ArrayType(colorRange);
	else {
	...
	if (normalize) for (let i = 0; i < flatArray.length; i++) flatArray[i] /= 255;
	return flatArray;
}
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/screen-grid-layer/screen-grid-layer-vertex.glsl.js
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/screen-grid-layer/screen-grid-layer-fragment.glsl.js
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/screen-grid-layer/screen-grid-layer-uniforms.js
var uniformBlock$8 = `\
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/screen-grid-layer/screen-grid-cell-layer.js
var ScreenGridCellLayer = class extends layer_default {
	getShaders() {
		return super.getShaders({
			...
			modules: [picking_default, screenGridUniforms]
		});
	}
	initializeState() {
	...
	updateState(params) {
		...
		} else if (oldProps.colorScaleType !== props.colorScaleType) updateColorRangeTexture(this.state.colorTexture, props.colorScaleType);
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/screen-grid-layer/bin-options-uniforms.js
var uniformBlock$7 = `\
uniform binOptionsUniforms {
  float cellSizePixels;
} binOptions;
`;
const binOptionsUniforms$3 = {
	name: "binOptions",
	vs: uniformBlock$7,
	uniformTypes: { cellSizePixels: "f32" }
};

//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/screen-grid-layer/screen-grid-layer.js
var defaultProps$4 = {
	cellSizePixels: {
		type: "number",
		value: 100,
...
var ScreenGridLayer = class extends aggregation_layer_default {
	...
	createAggregator(type) {
		if (type === "cpu" || !WebGLAggregator.isSupported(this.context.device)) return new CPUAggregator({
			dimensions: 2,
			getBin: {
				sources: ["positions"],
				getValue: ({ positions }, index, opts) => {
					...
					if (p[0] < 0 || p[0] >= viewport.width || p[1] < 0 || p[1] >= viewport.height) return null;
		...
		return new WebGLAggregator(this.context.device, {
			...
			...super.getShaders({
				modules: [project32_default, binOptionsUniforms$3],
				vs: `
  in vec3 positions;
  ...
  `
	...
	updateState(params) {
		...
		if (aggregatorChanged || changeFlags.dataChanged || changeFlags.updateTriggersChanged || changeFlags.viewportChanged || aggregation !== oldProps.aggregation || cellSizePixels !== oldProps.cellSizePixels) {
			const { width, height } = this.context.viewport;
			const { aggregator } = this.state;
			if (aggregator instanceof WebGLAggregator) aggregator.setProps({ binIdRange: [[0, Math.ceil(width / cellSizePixels)], [0, Math.ceil(height / cellSizePixels)]] });
			aggregator.setProps({
				pointCount: this.getNumInstances(),
				operations: [aggregation],
				binOptions: { cellSizePixels }
			});
		}
		if (changeFlags.viewportChanged) this.state.aggregator.setNeedsUpdate();
	...
	renderLayers() {
		...
		return new CellLayerClass(this.props, this.getSubLayerProps({ id: "cell-layer" }), {
			data: {
...
ScreenGridLayer.defaultProps = defaultProps$4;
var screen_grid_layer_default = ScreenGridLayer;

//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/common/utils/scale-utils.js
...
var AttributeWithScale = class {
	...
	getScalePercentile() {
		if (!this._percentile) this._percentile = applyScaleQuantile(getAttributeValue(this.input, this.inputLength));
		return this._percentile;
	}
	getScaleOrdinal() {
		if (!this._ordinal) this._ordinal = applyScaleOrdinal(getAttributeValue(this.input, this.inputLength));
	...
	getCutoff({ scaleType, lowerPercentile, upperPercentile }) {
		if (scaleType === "quantile") return [lowerPercentile, upperPercentile - 1];
		if (lowerPercentile > 0 || upperPercentile < 100) {
			...
			if (scaleType === "ordinal") {
				...
				if (highValue === -2) highValue = sortedUniqueValues.length - 1;
			}
	...
	update(props) {
		const oldProps = this.props;
		if (props.scaleType !== oldProps.scaleType) switch (props.scaleType) {
			case "quantile": {
		...
		if (props.scaleType !== oldProps.scaleType || props.lowerPercentile !== oldProps.lowerPercentile || props.upperPercentile !== oldProps.upperPercentile) this.cutoff = this.getCutoff(props);
		this.props = props;
		return this;
	}
};
...
function applyScaleOrdinal(values) {
	...
	for (const x of values) if (Number.isFinite(x)) uniqueValues.add(x);
	const sortedUniqueValues = Array.from(uniqueValues).sort();
	const domainMap = /* @__PURE__ */ new Map();
	for (let i = 0; i < sortedUniqueValues.length; i++) domainMap.set(sortedUniqueValues[i], i);
	return {
		attribute: {
...
function applyScaleQuantile(values, rangeLength = 100) {
	...
	while (++i < n) thresholds[i - 1] = threshold(sortedValues, i / n);
	return {
		attribute: {
...
function getAttributeValue(attribute, length) {
	...
	if (elementStride === 1) return value.subarray(0, length);
	const result = new Float32Array(length);
	for (let i = 0; i < length; i++) result[i] = value[i * elementStride + elementOffset];
	return result;
}
...
function threshold(domain, fraction) {
	...
	if (fraction <= 0 || domainLength < 2) return domain[0];
	if (fraction >= 1) return domain[domainLength - 1];
	const domainFraction = (domainLength - 1) * fraction;
	const lowIndex = Math.floor(domainFraction);
	const low = domain[lowIndex];
	return low + (domain[lowIndex + 1] - low) * (domainFraction - lowIndex);
}
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/common/utils/bounds-utils.js
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/hexagon-layer/hexbin.js
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/hexagon-layer/hexagon-cell-layer-vertex.glsl.js
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/hexagon-layer/hexagon-layer-uniforms.js
var uniformBlock$6 = `\
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/hexagon-layer/hexagon-cell-layer.js
var HexagonCellLayer = class extends column_layer_default {
	getShaders() {
		...
		return {
			...shaders,
			vs: hexagon_cell_layer_vertex_glsl_default
		};
	}
	initializeState() {
		super.initializeState();
	...
	draw({ uniforms }) {
		...
		if (fillModel.vertexArray.indexBuffer) fillModel.setIndexBuffer(null);
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/hexagon-layer/bin-options-uniforms.js
var uniformBlock$5 = `\
uniform binOptionsUniforms {
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/hexagon-layer/hexagon-layer.js
function noop$1() {}
var defaultProps$3 = {
	gpuAggregation: true,
	colorDomain: null,
...
var HexagonLayer = class extends aggregation_layer_default {
	getAggregatorType() {
		...
		if (gpuAggregation && WebGLAggregator.isSupported(this.context.device)) return "gpu";
		return "cpu";
	}
	createAggregator(type) {
		if (type === "cpu") {
			...
			return new CPUAggregator({
				...
				getBin: {
					sources: ["positions"],
					getValue: ({ positions }, index, opts) => {
						if (hexagonAggregator) return hexagonAggregator(positions, radius);
						const p = this.state.aggregatorViewport.projectPosition(positions);
		...
		return new WebGLAggregator(this.context.device, {
			...
			bufferLayout: this.getAttributeManager().getBufferLayouts({ isInstanced: false }),
			...super.getShaders({
				modules: [project32_default, binOptionsUniforms$2],
				vs: `
  in vec3 positions;
  in vec3 positions64Low;
  ...
  `
	...
	updateState(params) {
		...
		const { props, oldProps, changeFlags } = params;
		const { aggregator } = this.state;
		if ((changeFlags.dataChanged || !this.state.dataAsArray) && (props.getColorValue || props.getElevationValue)) this.state.dataAsArray = Array.from(createIterable(props.data).iterable);
		if (aggregatorChanged || changeFlags.dataChanged || props.radius !== oldProps.radius || props.getColorValue !== oldProps.getColorValue || props.getElevationValue !== oldProps.getElevationValue || props.colorAggregation !== oldProps.colorAggregation || props.elevationAggregation !== oldProps.elevationAggregation) {
			...
			if (dataAsArray) {
				...
				aggregator.setProps({ customOperations: [getColorValue && ((indices) => getColorValue(indices.map((i) => dataAsArray[i]), {
					indices,
					data: props.data
		...
		if (changeFlags.updateTriggersChanged && changeFlags.updateTriggersChanged.getColorValue) aggregator.setNeedsUpdate(0);
		if (changeFlags.updateTriggersChanged && changeFlags.updateTriggersChanged.getElevationValue) aggregator.setNeedsUpdate(1);
		return aggregatorChanged;
	}
	_updateBinOptions() {
	...
	draw(opts) {
		if (opts.shaderModuleProps.project) opts.shaderModuleProps.project.viewport = this.state.aggregatorViewport;
		super.draw(opts);
	}
	_onAggregationUpdate({ channel }) {
		...
		if (channel === 0) {
			...
			this.setState({ colors: new AttributeWithScale(result, aggregator.binCount) });
			props.onSetColorDomain(aggregator.getResultDomain(0));
		} else if (channel === 1) {
			const result = aggregator.getResult(1);
			this.setState({ elevations: new AttributeWithScale(result, aggregator.binCount) });
	...
	renderLayers() {
		...
		if (!colors || !elevations) return null;
		return new CellLayerClass(this.getSubLayerProps({ id: "cells" }), {
			data: {
				length: aggregator.binCount,
				attributes: {
					getBin: binAttribute,
...
HexagonLayer.defaultProps = defaultProps$3;
var hexagon_layer_default = HexagonLayer;

//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/contour-layer/marching-squares-codes.js
var HALF = .5;
var ONE6TH = 1 / 6;
var OFFSET = {
	N: [0, HALF],
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/contour-layer/marching-squares.js
function getVertexCode(weight, threshold$1) {
	if (Number.isNaN(weight)) return 0;
	if (Array.isArray(threshold$1)) {
		if (weight < threshold$1[0]) return 0;
		return weight < threshold$1[1] ? 1 : 2;
	}
	return weight >= threshold$1 ? 1 : 0;
}
function getCode(opts) {
	const { x, y, xRange, yRange, getValue, threshold: threshold$1 } = opts;
	...
	let top;
	let topRight;
	if (isLeftBoundary || isTopBoundary) top = 0;
	else {
		const w = getValue(x, y + 1);
		top = getVertexCode(w, threshold$1);
		weights += w;
	}
	if (isRightBoundary || isTopBoundary) topRight = 0;
	else {
		const w = getValue(x + 1, y + 1);
		topRight = getVertexCode(w, threshold$1);
		weights += w;
	}
	if (isRightBoundary || isBottomBoundary) right = 0;
	else {
		const w = getValue(x + 1, y);
		right = getVertexCode(w, threshold$1);
		weights += w;
	}
	if (isLeftBoundary || isBottomBoundary) current = 0;
	else {
		const w = getValue(x, y);
		current = getVertexCode(w, threshold$1);
		weights += w;
	}
	let code = -1;
	if (Number.isFinite(threshold$1)) code = top << 3 | topRight << 2 | right << 1 | current;
	if (Array.isArray(threshold$1)) code = top << 6 | topRight << 4 | right << 2 | current;
	let meanCode = 0;
	if (!isBoundary) meanCode = getVertexCode(weights / 4, threshold$1);
	return {
		code,
		meanCode
	};
...
function getPolygons(opts) {
	...
	if (!Array.isArray(offsets)) offsets = offsets[meanCode];
	const rX = x + 1;
	const rY = y + 1;
	const polygons = [];
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/contour-layer/contour-utils.js
function generateContours({ contours, getValue, xRange, yRange }) {
	...
	for (let i = 0; i < contours.length; i++) {
		...
		const { threshold: threshold$1 } = contour;
		for (let x = xRange[0] - 1; x < xRange[1]; x++) for (let y = yRange[0] - 1; y < yRange[1]; y++) {
			...
			if (Array.isArray(threshold$1)) {
				...
				for (const polygon of polygons) contourPolygons[polygonIndex++] = {
					vertices: polygon,
					contour
				};
			} else {
				const path = getLines(opts);
				if (path.length > 0) contourLines[segmentIndex++] = {
					vertices: path,
					contour
				};
			}
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/contour-layer/value-reader.js
...
function getAggregatorValueReader(opts) {
	...
	if (aggregator instanceof WebGLAggregator) {
		...
		if (buffer) return getWebGLAggregatorValueReader(new Float32Array(buffer.readSyncWebGL().buffer), binIdRange);
	}
	if (aggregator instanceof CPUAggregator) {
		const values = aggregator.getResult(channel)?.value;
		const ids = aggregator.getBins()?.value;
		if (ids && values) return getCPUAggregatorValueReader(values, ids, aggregator.binCount);
	}
	return null;
}
...
function getCPUAggregatorValueReader(values, ids, count$1) {
	const idMap = {};
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/contour-layer/bin-options-uniforms.js
var uniformBlock$4 = `\
uniform binOptionsUniforms {
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/contour-layer/contour-layer.js
var DEFAULT_COLOR = [
	255,
	255,
	255,
...
/** Aggregate data into a grid-based heatmap. The color and height of a cell are determined based on the objects it contains. */
var GridLayer$1 = class extends aggregation_layer_default {
	getAggregatorType() {
		return this.props.gpuAggregation && WebGLAggregator.isSupported(this.context.device) ? "gpu" : "cpu";
	}
	createAggregator(type) {
		if (type === "cpu") return new CPUAggregator({
			dimensions: 2,
			getBin: {
				sources: ["positions"],
				getValue: ({ positions }, index, opts) => {
		...
		return new WebGLAggregator(this.context.device, {
			...
			bufferLayout: this.getAttributeManager().getBufferLayouts({ isInstanced: false }),
			...super.getShaders({
				modules: [project32_default, binOptionsUniforms$1],
				vs: `
  in vec3 positions;
  in vec3 positions64Low;
  in float counts;
  ...
  `
	...
	updateState(params) {
		...
		if (!deepEqual(oldProps.contours, props.contours, 2)) this.setState({ contourData: null });
		return aggregatorChanged;
	}
	_updateBinOptions() {
		const bounds = this.getBounds();
	...
	_getContours() {
		...
		if (!aggregatedValueReader) return null;
	...
	renderLayers() {
		...
		return [lines && lines.length > 0 && new LinesSubLayerClass(this.getSubLayerProps({ id: "lines" }), {
			data: lines,
			...
			widthUnits: "pixels"
		}), polygons && polygons.length > 0 && new BandsSubLayerClass(this.getSubLayerProps({ id: "bands" }), {
			data: polygons,
			coordinateSystem: COORDINATE_SYSTEM.CARTESIAN,
			modelMatrix,
	...
	getPickingInfo(params) {
		const info = params.info;
		const { object } = info;
		if (object) info.object = { contour: object.contour };
		return info;
	}
};
GridLayer$1.layerName = "ContourLayer";
GridLayer$1.defaultProps = defaultProps$2;
var contour_layer_default = GridLayer$1;

//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/grid-layer/grid-cell-layer-vertex.glsl.js
var grid_cell_layer_vertex_glsl_default = `#version 300 es
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/grid-layer/grid-layer-uniforms.js
var uniformBlock$3 = `\
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/grid-layer/grid-cell-layer.js
var GridCellLayer = class extends column_layer_default {
	getShaders() {
		...
		return {
			...shaders,
			vs: grid_cell_layer_vertex_glsl_default
		};
	}
	initializeState() {
		super.initializeState();
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/grid-layer/bin-options-uniforms.js
var uniformBlock$2 = `\
uniform binOptionsUniforms {
  vec2 cellOriginCommon;
  vec2 cellSizeCommon;
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/grid-layer/grid-layer.js
function noop() {}
var defaultProps$1 = {
	gpuAggregation: true,
	colorDomain: null,
		...
		if (type === "cpu") {
			...
			return new CPUAggregator({
				...
				getBin: {
					sources: ["positions"],
					getValue: ({ positions }, index, opts) => {
						if (gridAggregator) return gridAggregator(positions, cellSize);
						const p = this.state.aggregatorViewport.projectPosition(positions);
						const { cellSizeCommon, cellOriginCommon } = opts;
						return [Math.floor((p[0] - cellOriginCommon[0]) / cellSizeCommon[0]), Math.floor((p[1] - cellOriginCommon[1]) / cellSizeCommon[1])];
					}
...
GridLayer.defaultProps = defaultProps$1;
...
function boundsContain(currentBounds, targetBounds) {
	if (targetBounds[0] >= currentBounds[0] && targetBounds[2] <= currentBounds[2] && targetBounds[1] >= currentBounds[1] && targetBounds[3] <= currentBounds[3]) return true;
	return false;
}
var scratchArray = new Float32Array(12);
function packVertices(points, dimensions = 2) {
	let index = 0;
	for (const point of points) for (let i = 0; i < dimensions; i++) scratchArray[index++] = point[i] || 0;
	return scratchArray;
}
function scaleToAspectRatio(boundingBox, width, height) {
	...
	if (currentWidth / currentHeight < width / height) newWidth = width / height * currentHeight;
	else newHeight = height / width * currentWidth;
	if (newWidth < width) {
		newWidth = width;
		newHeight = height;
	}
...
var triangle_layer_vertex_glsl_default = `\
#version 300 es
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/heatmap-layer/triangle-layer-fragment.glsl.js
var triangle_layer_fragment_glsl_default = `\
#version 300 es
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/heatmap-layer/triangle-layer-uniforms.js
var uniformBlock$1 = `\
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/heatmap-layer/triangle-layer.js
var TriangleLayer = class extends layer_default {
	getShaders() {
		return super.getShaders({
			...
			modules: [project32_default, triangleUniforms]
		});
	}
	...
	draw() {
		...
		const { aggregationMode, colorDomain, intensity, threshold: threshold$1, colorTexture, maxTexture, weightsTexture } = this.props;
		const triangleProps = {
			aggregationMode,
			colorDomain,
			intensity,
...
function filterProps(props, filterKeys) {
	const filteredProps = {};
	for (const key in props) if (!filterKeys.includes(key)) filteredProps[key] = props[key];
...
var AggregationLayer = class extends composite_layer_default {
	...
	updateState(opts) {
		...
		if (changeFlags.extensionsChanged) {
			...
			if (shaders && shaders.defines) shaders.defines.NON_INSTANCED_MODEL = 1;
	...
	**/
	isAggregationDirty(updateOpts, params = {}) {
		...
		if (changeFlags.dataChanged) return true;
		if (updateTriggersChanged) {
			if (updateTriggersChanged.all) return true;
			for (const accessor of accessors) if (updateTriggersChanged[accessor]) return true;
		}
		if (compareAll) {
			if (changeFlags.extensionsChanged) return true;
			return compareProps({
				oldProps,
				newProps: props,
				ignoreProps,
		...
		for (const name of dataProps) if (props[name] !== oldProps[name]) return true;
		return false;
	}
	/**
	...
	**/
	isAttributeChanged(name) {
		...
		if (!name) return !isObjectEmpty(changedAttributes);
...
var aggregation_layer_default$1 = AggregationLayer;
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/heatmap-layer/weights-fs.glsl.js
var weights_fs_glsl_default = `\
#version 300 es
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/heatmap-layer/max-vs.glsl.js
var max_vs_glsl_default = `\
#version 300 es
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/heatmap-layer/max-fs.glsl.js
var max_fs_glsl_default = `\
#version 300 es
in vec4 outTexture;
out vec4 fragColor;
void main() {
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/heatmap-layer/heatmap-layer-uniforms.js
var uniformBlock = `\
...
//#endregion
//#region node_modules/@deck.gl/aggregation-layers/dist/heatmap-layer/heatmap-layer.js
var RESOLUTION = 2;
var TEXTURE_PROPS = {
	format: "rgba8unorm",
	dimension: "2d",
...
var FLOAT_TARGET_FEATURES = ["float32-renderable-webgl", "texture-blend-float-webgl"];
var DIMENSIONS = { data: { props: ["radiusPixels"] } };
/** Visualizes the spatial distribution of data. */
var HeatmapLayer = class extends aggregation_layer_default$1 {
	getShaders(shaders) {
		let modules = [project32_default];
		if (shaders.modules) modules = [...modules, ...shaders.modules];
		return super.getShaders({
			...shaders,
			modules
		});
	...
	_updateHeatmapState(opts) {
		...
		} else if (changeFlags.viewportZoomChanged) this._debouncedUpdateWeightmap();
		if (props.colorRange !== oldProps.colorRange) this._updateColorTexture(opts);
		if (this.state.isWeightMapDirty) this._updateWeightmap();
		this.setState({ zoom: opts.context.viewport.zoom });
	}
	renderLayers() {
		const { weightsTexture, triPositionBuffer, triTexCoordBuffer, maxWeightsTexture, colorTexture, colorDomain } = this.state;
		const { updateTriggers, intensity, threshold: threshold$1, aggregation } = this.props;
		return new (this.getSubLayerClass("triangle", triangle_layer_default))(this.getSubLayerProps({
			id: "triangle-layer",
			updateTriggers
		}), {
	...
	_getChangeFlags(opts) {
		...
		if (!opts.context.viewport || opts.context.viewport.zoom !== zoom) changeFlags.viewportZoomChanged = true;
		return changeFlags;
	}
	_createTextures() {
	...
	_setupTextureParams() {
		...
		if (!floatTargetSupport) log_default.warn(`HeatmapLayer: ${this.id} rendering to float texture not supported, falling back to low precision format`)();
	}
	...
	_setupResources() {
		...
		maxWeightTransform.model.shaderInputs.setProps({ maxWeight: maxWeightProps });
		this.setState({
			weightsTexture,
			maxWeightsTexture,
			maxWeightTransform,
	...
	_updateWeightmap() {
		...
		const commonBounds = this._worldToCommonBounds(worldBounds, { useLayerCoordinateSystem: true });
		if (colorDomain && aggregation === "SUM") {
			const { viewport: viewport$1 } = this.context;
			const metersPerPixel = viewport$1.distanceScales.metersPerUnit[2] * (commonBounds[2] - commonBounds[0]) / textureSize;
			this.state.colorDomain = colorDomain.map((x) => x * metersPerPixel * weightsScale);
		} else this.state.colorDomain = colorDomain || DEFAULT_COLOR_DOMAIN;
...
//#endregion
export { CPUAggregator, contour_layer_default as ContourLayer, grid_layer_default as GridLayer, heatmap_layer_default as HeatmapLayer, hexagon_layer_default as HexagonLayer, screen_grid_layer_default as ScreenGridLayer, WebGLAggregator, aggregation_layer_default as _AggregationLayer };
